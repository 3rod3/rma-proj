...
if clientID!=-1:
    ...
    #handle de sensores e reitura sensores
        returnCode, l_sensor = sim.simxGetObjectHandle(clientID, 'psensor0', sim.simx_opmode_oneshot_wait)
        print('Handle return code:',(returnCode, l_sensor))
        returnCode, r_sensor = sim.simxGetObjectHandle(clientID, 'psensor1', sim.simx_opmode_oneshot_wait)
        print('Handle return code:',(returnCode, r_sensor))
        returnCode, f_sensor = sim.simxGetObjectHandle(clientID, 'psensor2', sim.simx_opmode_oneshot_wait)
        print('Handle return code:',(returnCode, f_sensor))


        [returnCode, detectionState0, [detectpx0,detectpy0,detectpz0], detectedObjectHandle, detectedSurfaceNormalVector] = sim.simxReadProximitySensor(clientID, l_sensor, sim.simx_opmode_oneshot_wait)
        
        detectpx0 = float("%0.2f" %(detectpx0))
        detectpy0 = float("%0.2f" %(detectpy0))
        detectpz0 = float("%0.2f" %(detectpz0))
        print("return code = ", returncode, "dectetionstate = ",detectionState0, "detectedPoint = ",[detectpx0,detectpy0,detectpz0],
              "detectedObjectHandle = ", detectedObjectHandle," detectedSurfaceNormalVector = ",detectedSurfaceNormalVector )
        
        
        
        [returnCode, detectionState1, [detectpx1,detectpy1,detectpz1], detectedObjectHandle, detectedSurfaceNormalVector] = sim.simxReadProximitySensor(clientID, r_sensor, sim.simx_opmode_oneshot_wait)
        
        detectpx1 = float("%0.2f" %(detectpx1))
        detectpy1 = float("%0.2f" %(detectpy1))
        detectpz1 = float("%0.2f" %(detectpz1))
        print("return code = ", returncode, "dectetionstate = ",detectionState1, "detectedPoint = ",[detectpx1,detectpy1,detectpz1],
              "detectedObjectHandle = ", detectedObjectHandle," detectedSurfaceNormalVector = ",detectedSurfaceNormalVector )
        
        
        [returnCode, detectionState2, [detectpx2,detectpy2,detectpz2], detectedObjectHandle, detectedSurfaceNormalVector] = sim.simxReadProximitySensor(clientID, f_sensor, sim.simx_opmode_oneshot_wait)
        
        detectpx2 = float("%0.2f" %(detectpx2))
        detectpy2 = float("%0.2f" %(detectpy2))
        detectpz2 = float("%0.2f" %(detectpz2))
        print("return code = ", returncode, "dectetionstate = ",detectionState2, "detectedPoint = ",[detectpx2,detectpy2,detectpz2],
          "detectedObjectHandle = ", detectedObjectHandle," detectedSurfaceNormalVector = ",detectedSurfaceNormalVector )
        
        # [returncode, [px1,py1,pz1]] = sim.simxGetObjectPosition(clientID, robotHandle, -1, sim.simx_opmode_oneshot_wait)
        # sim.simxSetJointTargetVelocity(clientID, r_wheel, v1, sim.simx_opmode_oneshot_wait)
        # sim.simxSetJointTargetVelocity(clientID, l_wheel, v1, sim.simx_opmode_oneshot_wait)
            ...
            while (detectionState0 == True and detectpz0 > 0.55) or (detectionState1 == True and detectpz1 > 0.55):
                ...
                if detectpz0 > 0.55:
                    ...
                    while abs(ang_ori) >= 2:
                        ...
                        sim.simxSetJointTargetVelocity(clientID, r_wheel, v1+(kp*v2), sim.simx_opmode_oneshot_wait)
                        sim.simxSetJointTargetVelocity(clientID, l_wheel, v1-(kp*v2), sim.simx_opmode_oneshot_wait)
                        print('ang orient = ',ang_ori)
        ...
        if (detectionState2 == True and detectpz2<0.3):
                ...
                if detectionState0 == True and detectionState1 == True:
                    ...
                    while abs(ang_ori - 180) > 3:
                        ...
                        sim.simxSetJointTargetVelocity(clientID, r_wheel, v, sim.simx_opmode_oneshot_wait)
                        sim.simxSetJointTargetVelocity(clientID, l_wheel, -v, sim.simx_opmode_oneshot_wait)
                        print('ang orient = ',ang_ori)
